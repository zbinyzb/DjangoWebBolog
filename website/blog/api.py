...
def zbin_checkperm(request):
    ...
    if user_token:
        ...
        for p in permissions:
            ...
            parm_str = app_str + '.' + p + '_' + model_str
            # print(parm_str)
            check = user.has_perm(parm_str)
            # print(check)
            if check == False:
...
#自动登录
@api_view(['POST'])
def zbin_autologin(request):
    token = request.POST['token']
    # 选用filter查询防止与登出对token值的清除操作互相干扰
    user_token = Token.objects.filter(key=token)
    # print(user_token)
    if user_token:
...
def zbin_group(requests):
    ...
    if requests.method == "POST":
        ...
        if checkUser != 'perm_pass':
            return Response(checkUser)
        group_name = requests.POST['group']
        userlist_name = json.loads(requests.POST['userlist'])
    ...
    if requests.method == "PUT":
        ...
        new_name = requests.POST['new_group']
        perm_list = json.loads(requests.POST['perm_list'])
...
def zbinPinglun(request):
    if request.method == "GET":
        ...
        article = Article.objects.get(id=article_id)
        # 倒转一下列表，使新发评论位于首位
        pingluns = Pinlun.objects.filter(belong=article)[::-1]
        # 分页
        total = len(pingluns)
    ...
    if request.method == "POST":
        ...
        new_pinglun = Pinlun(belong_user=user,belong=article,text=text)
        new_pinglun.save()
        return Response('ok') 
#点赞收藏打赏的动态信息
@api_view(['POST'])
